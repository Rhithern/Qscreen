...
// POST /api/admin/jobs/[id]/publish - Publish job (set status to 'live')
export async function POST(
  ...
  try {
    ...
    if (existingJob.status === 'live') {
      return NextResponse.json(
        createErrorResponse('ALREADY_PUBLISHED', 'Job is already published'),
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        createErrorResponse('DATABASE_ERROR', 'Failed to publish job'),
        { status: 500, headers: corsHeaders }
      );
    }

    return NextResponse.json(
      createSuccessResponse({ id, status: 'live' }),
      { headers: corsHeaders }
    );

  } catch (error) {
    console.error('Error in POST /api/admin/jobs/[id]/publish:', error);
    return NextResponse.json(
